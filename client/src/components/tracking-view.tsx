...
export default function TrackingView() {
  ...
  const calculateTotals = () => {
    ...
    items.forEach((item) => {
      totals.calories += item.nutrition.Calories * (item.servings || 1);
      totals.protein += item.nutrition.Protein * (item.servings || 1);
      totals.carbs +=
        item.nutrition["Total Carbohydrate"] * (item.servings || 1);
      totals.fat += item.nutrition["Total Fat"] * (item.servings || 1);
      totals.sodium += item.nutrition.Sodium * (item.servings || 1);
      totals.fiber += item.nutrition.Fiber * (item.servings || 1);
    });
